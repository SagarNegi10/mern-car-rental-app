...
export const checkAvailabilityOfCar = async (req, res) => {
    try {
        ...
        res.json({sucsses: true, availableCars})

    } catch (error) {
        console.log(error.message);
        res.json({sucsses: false, message: error.message})
    }
}
...
export const createBooking = async (req, res) => {
    try {
        ...
        if(!isAvailable){
            return res.json({sucsses: false, message: "Car is not Available"})
        }
        ...
        res.json({sucsses: true, message: "Booking created"})
...
export const getUserBookings = async (req, res) => {
    try {
        ...
        res.json({sucsses: true, bookings})
...
export const getOwnerBookings = async (req, res) => {
    try {
        if(req.user.role != 'owner'){
            return res.json({sucsses: false, message: "Unauthorized"})
        }
...
export const changeBookingStatus = async (req, res) => {
    try {
        ...
        res.json({sucsses: true, message: "Status Updated"})
